

  <div id="into_container">
    <h3>Introduction</h3>
    <p class="paragraphe">
      Ce petit tutoriel a pour but de metre en place une première Div et d'expliquer les bases de lalibrairie.<br>
      Tous d'abord ajouter My Div.js à votre projet en écrivant cette ligne :
      <pre class="code">
        <br>
        <spam >Depuis le site web :</spam>
        <span>< script type="text/javascript" src="https://www.mydiv.js.org/MyDivDev.js" > </ script ></span><br>
        <spam>Ou en local :</spam>
        <span>< script type="text/javascript" src="MyDivDev.js" > </ script ></span>
      </pre>

    </p>
  </div>
